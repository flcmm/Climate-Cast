...
export default function Welcome () {
  ...
  const weatherApiQueryURl = `http://api.weatherapi.com/v1${currWeatherParams}?key=${weatherApiKey}&q=Manila`
  useEffect(() => {
  ...
  return (
    <div className='flex flex-col w-full h-screen'>
      <div className='w-full flex flex-row p-3 justify-between items-center'>
        ...
        <h1 className='font-bold text-2xl'>{data ? data.location.name : '- '} {data ? data.location.region : '- '}, {data ? data.location.country : '- '}</h1>
      ...
      <div className='w-full flex items-center justify-center'>
        <p className='px-3 py-2 bg-black rounded-3xl text-white font-semibold'>{data ? data.location.localtime : '- - - -'}</p>
      </div>

      <div className='grid place-content-center'>
        <h1 className='text-black font-bold'>{data ? data.current.condition.text : '- '}</h1>
      </div>

      <div className='grid place-content-center'>
        <h1 className='font-bold text-9xl'>{data ? data.current.temp_c : '- '}°</h1>
      </div>
        ...
        <div className='bg-black w-3/4 p-3 flex flex-row justify-around rounded-lg'>
          <div className='flex w-1/4 gap-5 justify-center items-center flex-col'>
            ...
            <div className='flex flex-col justify-center items-center'>
              <p className='text-white'>{data ? data.current.wind_kph : '- '} kph</p>
          ...
          <div className='flex w-1/4 justify-center items-center flex-col'>
            ...
            <div className='flex flex-col justify-center items-center'>
              <p className='text-white'>{data ? data.current.feelslike_c : '- '}%</p>
          ...
          <div className='flex w-1/4 justify-center items-center  flex-col'>
            ...
            <div className='flex flex-col justify-center items-center'>
              <p className='text-white'>{data ? data.current.uv : '- '}</p>
